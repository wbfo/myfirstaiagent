  ...
  return async (ctx, next) => {
    ...
    try {
      ...
      if (ackReaction) {
        ...
        if (isBusy) {
          logVerbose(
            `telegram preflight: chat ${chatId} (key ${sequentialKey}) is busy; signaling...`,
          );
          ...
          await withTelegramApiErrorLogging({
            ...
            runtime,
            fn: () =>
              ctx.api.setMessageReaction(chatId, msg.message_id, [
                { type: "emoji", emoji: busyEmoji as any },
              ]),
          }).catch(() => {});
        } else {
          ...
          await withTelegramApiErrorLogging({
            ...
            fn: () =>
              ctx.api.setMessageReaction(chatId, msg.message_id, [
                { type: "emoji", emoji: ackReaction as any },
              ]),
          }).catch(() => {});
        }
      }
