...
export async function detectAndPullLocalModels(): Promise<void> {
  ...
  try {
    ...
    for (const model of requiredModels) {
      if (output.includes(model.id)) {
        ...
        try {
          ...
          log.error(
            `Failed to automatically pull ${model.id}. You may need to run 'ollama pull ${model.id}' manually.`,
          );
        }
      }
    }
    ...
    log.error(
      "Failed to detect Ollama environment. Is the Ollama app running? Expected for costOptStack.",
    );
  }
}
