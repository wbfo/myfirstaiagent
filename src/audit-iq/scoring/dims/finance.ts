...
export function scoreFinance(brief: AuditBrief): DimensionScore {
  ...
  if (!brief.finance.hasCashFlowVisibility) {
    ...
    findings.push({
      ...
      detail:
        "Operating without real-time cash position awareness is the leading cause of preventable business failure.",
      remediation:
        "Implement a weekly cash flow dashboard. QuickBooks, Xero, or even a spreadsheet updated Monday morning.",
    });
  }
  ...
  if (!brief.finance.hasMonthlyClose) {
    ...
    findings.push({
      ...
      remediation:
        "Establish a monthly close checklist and complete it within 10 business days of month-end.",
    });
  }
  ...
  if (burn !== undefined && burn < 6) {
    ...
    findings.push({
      ...
      detail:
        "Less than 6 months of runway creates existential pressure and limits strategic options.",
