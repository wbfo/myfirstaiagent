...
export function scoreSystems(brief: AuditBrief): DimensionScore {
  ...
  if (!brief.systems.hasCrm) {
    ...
    findings.push({
      ...
      detail:
        "Relationships and pipeline managed in a spreadsheet or memory do not scale past 20 active prospects.",
      remediation:
        "Implement HubSpot Free or Notion CRM. Migrate all active leads within one week.",
    });
  }
  ...
  if (!brief.systems.hasDataBackup) {
    ...
    findings.push({
      ...
      detail:
        "One hardware failure or ransomware event could destroy years of work with no recovery path.",
      remediation:
        "Enable automated cloud backup (Google Drive, Backblaze, or AWS S3) by end of week. Verify a restore.",
    });
  }
