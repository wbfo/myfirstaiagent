...
export async function collectPluginsTrustFindings(params: {
    ...
    const npmPluginInstalls = Object.entries(pluginInstalls).filter(
        ([, record]: [string, any]) => record?.source === "npm",
    );
    if (npmPluginInstalls.length > 0) {
        const unpinned = npmPluginInstalls
            .filter(([, record]: [string, any]) => typeof record.spec === "string" && !isPinnedRegistrySpec(record.spec))
            .map(([pluginId, record]: [string, any]) => `${pluginId} (${record.spec})`);
        ...
        const missingIntegrity = npmPluginInstalls
            .filter(
                ([, record]: [string, any]) => typeof record.integrity !== "string" || record.integrity.trim() === "",
            )
        ...
        for (const [pluginId, record] of npmPluginInstalls as any[]) {
    ...
    if (npmHookInstalls.length > 0) {
        const unpinned = npmHookInstalls
            .filter(([, record]: [string, any]) => typeof record.spec === "string" && !isPinnedRegistrySpec(record.spec))
            .map(([hookId, record]: [string, any]) => `${hookId} (${record.spec})`);
        if (unpinned.length > 0) {
            findings.push({
        ...
        for (const [hookId, record] of npmHookInstalls as any[]) {
            const recordedVersion = record.resolvedVersion ?? record.version;
            if (!recordedVersion) {
                continue;
            }
